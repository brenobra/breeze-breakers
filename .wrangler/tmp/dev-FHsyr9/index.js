...
// .wrangler/tmp/bundle-i0WQql/checked-fetch.js
...
// .wrangler/tmp/bundle-i0WQql/strip-cf-connecting-ip-header.js
...
var src_default = {
  async fetch(request, env, ctx) {
    ...
    if (pathname === "/api/breakers" && request.method === "POST") {
      try {
        const newBreaker = await request.json();
        ...
        const { success } = await env.DB.prepare("INSERT INTO breakers (number, amperage, description, room) VALUES (?, ?, ?, ?)").bind(newBreaker.number, newBreaker.amperage, newBreaker.description, newBreaker.room).run();
        if (success) {
...
// .wrangler/tmp/bundle-i0WQql/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-i0WQql/middleware-loader.entry.ts
